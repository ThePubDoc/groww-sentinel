...
- **D-09:** Detect a distorted average cost via a **quantity jump vs stored prior `qty`** without a proportional cost-basis change (bonus/split signature). Concrete heuristic (from RESEARCH): **qty grew >5% AND invested capital `qty*avg_cost` changed <5%** → corp action (a real AVERAGE buy moves capital; a bonus/split doesn't). On detection, that run **replaces the P&L-based flag (STOP/BOOK/AVERAGE) with a `CORP-ACTION` warning line** (suppress the `pct` display, like NO PRICE) so a distorted cost can't fire a false trade. Weight-based TRIM still evaluates. Detection input (prior qty/avg_cost) comes from state → keep `rules.py` pure by passing it in.
- **D-11:** On corp-action detection, **rescale the stored peak** `peak *= prior_qty/qty` (from RESEARCH). A bonus/split drops the price level, so an un-rescaled pre-split peak would fire a phantom TRAIL WATCH forever. Rescale before the below-peak comparison.
- **D-12:** Day-change baseline must select the **most recent snapshot with `date < today`**, NOT `sorted(keys)[-2]` — hourly reruns overwrite today's key, so `[-2]` would wrongly grab today's earlier value on the 2nd+ run (RESEARCH off-by-one catch).
